...
date: 2026-08-30
version: v2.0
status: 待执行（2026-08-30用户确认Q1-Q5，纳入370号方案统一实施）
related:
...
## 五、前置任务清单（2026-08-30 已全部确认）

| # | 问题 | 选项 | 2026-08-30 确认结论 |
|---|------|------|-------------------|
| Q1 | JUD-2的共识聚合路径 | A)直接消费dim_results_json B)仍通过StatusEngine重新计算 | **确认Path C**：JUD通过StatusEngine.evaluate()调用dim引擎重新计算，不消费dim_results_json |
| Q2 | 十二维体系(B组DIM-6~12)是否在JUD中独立计算？ | | **dim1-dim7有引擎，B组5维从pre_feat_cache直读**（当前状态，后续可补充引擎） |
| Q3 | SIG-6复合引擎是否保留？ | | **不保留**：SIG为5引擎（缠论/量价/筹码/情绪/因子） |
| Q4 | 366号方案步骤7-10是否阻塞本次贯通？ | | **不阻塞**：369号已完成dim引擎物理合入，步骤7-10需重新评估 |
| Q5 | 七维描述的"冲突问题"是否需要在本次方案中解决？ | | **取消旧七维描述**：`build_seven_dim_report()`及6个builder将被删除，SIG直接产出`seven_dim_json`替代 |

> **v2.0更新说明**：本方案的5项待确认问题已于2026-08-30全部确认。实施细节已纳入370号方案（JUD环节独立化实施计划），本方案不再独立执行。
...
| 版本 | 日期 | 修订内容 |
|------|------|---------|
| v1.0 | 2026-08-23 | 初版：5阶段贯通方案+5项待确认问题 |
| **v2.0** | **2026-08-30** | **Q1-Q5全部确认：Path C确认、SIG-6取消、旧七维描述取消。实施纳入370号方案** |
