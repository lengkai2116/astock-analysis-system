...
date: 2026-09-19
version: v1.0（465-1 打分设计、465-2 date 回填、465-3 audit 误判已实施；465-4~465-8 已于 479-10 拍板处置——465-4/5/6/7 关闭 + 465-8 落地）
status: ✅ 全部实施完成（465-8 于 2026-09-24 经 479-10 落地；465-4~7 关闭）
related:
...
- **处置（2026-09-24 经 479-10 关闭）**：dim2 `_structure_plain` 已删、各维 `plain` 键废除（dim8 唯一叙事口径，436 共识）。dim8 structure.text 已产「缠论方向/阶段/趋势依据」趋势结论句（探针实证），缺陷天然消解，无需独立立项。
...
- **处置（2026-09-24 经 479-10 关闭）**：`_assess_vs_indicator`（RSI 强弱 + 均线形态）值已入 dim8 `_DIM8_E_FIELDS['structure']` evidence（探针实证 evidence 含「RSI 34 中性」），已覆盖。
...
- **处置（2026-09-24 经 479-10 关闭）**：446 重构后单级 judgment＝价格vs中枢主判据；探针 8 股 `judgment.structure` 与 `multi_level_direction_text` 全部一致（茅台由「上升」改判「下降」，原「上升 vs 周线降日线反弹」矛盾消解）。且多级别方向属 SIG 如实并列、判定归 JUD（439 边界），符合架构。
...
- **处置（2026-09-24 经 479-10 关闭）**：实证万科 divergence=顶背驰(趋势背驰,down) 与一买（下跌趋势背驰）同在下跌维度、方向一致，非真冲突；audit「背驰检测 / 有确认买点」两条件已如实并列，判定归 JUD。符合「SIG 不仲裁、如实并列现状」标准。
...
- **处置（已实施，2026-09-24 经 479-10）**：RSI 分档旧三档（≥70偏强/≤30偏弱/else中性）→ 新五档：`≥70 偏强 / ≥60 偏强-中性 / ≤30 偏弱 / ≤40 偏弱-中性 / else 中性`——弱侧（30-40）与强侧（60-70）不再吞为「中性」。实证 600519 RSI34→偏弱-中性、601318 RSI38→偏弱-中性；300750 RSI24 偏弱、000002 RSI77 偏强保持；中位值股仍「中性」。
- **验证**：`tests/test_442_vs_indicator.py` 更新 test_ma_mixed_with_rsi（RSI 68→51 中性）+ 新增 `test_4658_five_bins`（5 档值域）、`test_4658_threshold_boundaries`（70/60/40/30 阈值边界）；单测 **45 passed** + 回归 **87 passed** + 8 股真实数据探针全绿（详见 479 方案 §七 479-10 实施记录）。
