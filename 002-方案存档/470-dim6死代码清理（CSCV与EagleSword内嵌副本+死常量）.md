...
- ~~dim6 仍挂账：audit「无高风险事件」只查 severity=='极高' 口径（445 冻结，需拍板）、PIERS-E roce=0.0 误触发（数据`因`层，另号核查数据源）。~~ → **✅ 2026-09-26 全部关闭**：audit 口径已拍板 A 维持（475 P5 键名「无极高风险事件」+ dim6 定稿 4B 同判据，详见 471 待办段）；PIERS-E roce 已由 472/473/474 闭环（禁假值+标准口径 EBIT/(总资产-流动负债)，484 verify 实证）。
