...
> **2026-09-15 补充（dim8 归属裁定）**：用户就 dim8 拍板三点定位共识——dim8 作为 **SIG 现状描述统一整理模块**（收集 dim2-dim7 现状→整理→输出 OUT），作为 dim8 后续修正的标准。本报告 §3.2/§八陈述的"当前 dim8 运行于 JUD、非 OUT 文字通路生产者"是**现状**属实；但在 SIG 文字类生产者上**未达共识**（现仍为 `generate_seven_dim_from_signals`，非 dim8）。420 号同日被裁定"定位错误·否决"（增强的是 JUD 内部 summary，非 SIG 现状描述）。后续"dim8 归 SIG 整理现状"作为目标，当前代码未达成，实施须另开/修订方案。
