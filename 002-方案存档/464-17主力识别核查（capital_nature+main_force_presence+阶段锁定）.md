...
status: ✅ 已完成（三方向实施 + 464-13 置信门槛≥0.3 拍板实施，2026-09-21；135+25 测试全绿）
related:
...
### 464-13 置信门槛（✅ 已拍板实施 2026-09-21：≥0.3）
原实证（修复前 3962 只）：均值 0.458 / 中位 0.468 / 最低 0.136；≥0.3 拦 28%、≥0.2 拦 5%、≥0.5 拦 59%。

**修复后重估**（2026-09-18 旧 pre_feat 4572 只 + 分库真实证据，按修复后 _consensus 修正逻辑反推）：
| 指标 | 修复前 | 修复后 |
|---|---|---|
| 均值 | 0.444 | 0.525 |
| 中位 | 0.455 | 0.528 |
...
presence 分组：有证据组（strong/risk/moderate）均值 **0.587** vs none 组 **0.439**——置信可区分主力在场，软修正生效。

**拍板**：门槛 **≥0.3**；低置信 → audit 条件 1 satisfied=False；tags 兜底路径（无置信键）仅看 phase 不额外拦截。实施：phase_info 透传 confidence + audit 条件 1 加 `_phase_conf >= 0.3`（dim4_chip_fund_engine.py evaluate）；新增 test_464_17.py 6 用例。**daemon 重算后可用真实新 pre_feat 复核本模拟**（模拟误差主要来自 capital_nature 修复后分类的 LHB total_buy 近似，≤±2% 拦截率）。
