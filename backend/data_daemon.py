    ...
    with _flask_app.app_context():
        ...
        def _raw2_one(code):
            ...
            try:
                ...
                # 财务健康指标
                # 472号：ROCE 禁假值 + EBIT 回填（Tushare fina_indicator 不返回 roce 字段；
                #   finance_report_cache 已有真实 ebit，balancesheet 有 total_assets/total_liab/current_liab）。
                # 474号：口径切换为主定义 EBIT/(总资产-流动负债)（对齐 dim4/dim7/finance_report_service 消费侧）；
                #   current_liab 缺失时回退 EBIT/(总资产-总负债) → fina.roe×1.2（经验系数，ROCE 略高于 ROE）→ 缺数据不产键。
                try:
                    ...
                    # 474号：口径切换为主定义 EBIT/(总资产-流动负债)；current_liab 缺失回退 EBIT/(总资产-总负债)
                    _capital = None
                        ...
                        try:
                            ...
                            if _ta is not None and _cur_liab is not None and _ta == _ta and _cur_liab == _cur_liab:
                                ...
                                if _ta is not None and _tl is not None and _ta == _ta and _tl == _tl:
                                    _capital = float(_ta) - float(_tl)
                        except (TypeError, ValueError):
                            _capital = None
                    _roce = None
                    if _ebit is not None and _capital is not None and _capital > 0:
                        _roce = round(_ebit / _capital * 100, 2)
