...
def fetch_real_stock_data(ts_code, db_path, num_klines=300):
    """从DuckDB获取真实股票数据（取最近num_klines条）"""
    ...
    # 取最近的num_klines条
    df = df.tail(num_klines).reset_index(drop=True)
...
def plot_chanlun(ts_code, klines_raw, klines_merged, fractals, strokes, segments, pivot, output_file):
    ...
    for k in klines_raw:
        ax1.plot([k.idx, k.idx], [k.low, k.high], color='#3a3a4e', linewidth=1.2, alpha=0.4, zorder=1)
    
    for k in klines_merged:
        ax1.plot([k.idx, k.idx], [k.low, k.high], color='#8a8aae', linewidth=2.2, zorder=2)
    
    for f in fractals:
        if f['type'] == 'top':
            ax1.scatter(f['idx'], f['high'], marker='v', color='#ff6b6b', s=300, zorder=6, edgecolors='white', linewidth=2)
            ax1.annotate('TOP', (f['idx'], f['high']), textcoords="offset points", xytext=(0, 15), fontsize=11, color='#ff6b6b', ha='center', fontweight='bold')
        else:
            ax1.scatter(f['idx'], f['low'], marker='^', color='#4ecdc4', s=300, zorder=6, edgecolors='white', linewidth=2)
    ...
    ax1.set_title(f'CHANLUN Complete Structure - {ts_code} (Last {len(klines_raw)} K-lines)', fontsize=18, pad=25, color='white', fontweight='bold')
    ...
    ax2.set_title(f'Step 1: Contain Relation Processing ({len(klines_raw)} -> {len(klines_merged)} K-lines)', fontsize=13, pad=12, color='white', fontweight='bold')
...
def main():
    ...
    print("="*70)
    print("CHANLUN DIAGRAM GENERATOR - Real Data Version")
    print(f"Stock Code: {ts_code} (Last {num_klines} K-lines)")
    print("Based on: 《一小时漫画缠论实战法》")
    print("="*70)
    
    print("\n[Step 1] Fetching real stock data from cache...")
    klines_raw = fetch_real_stock_data(ts_code, db_path, num_klines)
    print(f"    -> Fetched {len(klines_raw)} K-lines")
    ...
    output_file = f'/Users/kalence/Desktop/测试/chanlun_{ts_code}_recent{num_klines}.png'
    plot_chanlun(ts_code, klines_raw, klines_merged, fractals, strokes, segments, pivot, output_file)
    ...
    for i, s in enumerate(strokes, 1):
        ...
        print(f"  #{i}: {direction:5s} | {k_count} K-lines | Price: {s['start_price']:.2f} -> {s['end_price']:.2f}")
    
    print(f"\nSegments: {len(segments)}")
    print(f"Central Pivot: {'Found' if pivot else 'Not found'}")
    if pivot:
        print(f"  ZG = {pivot['ZG']:.2f}, ZD = {pivot['ZD']:.2f}, Range = {pivot['ZG'] - pivot['ZD']:.2f}")
    print("="*70)
