...
class CacheManager:
    def __init__(self, read_only=False):
        ...
        try:
            ...
            logger.warning(f"无法连接到数据库文件，使用内存模式: {e}")
    ...
    def clear_old_cache(self, days=30):
        ...
        with self._lock:
            self.conn.execute("DELETE FROM daily_cache WHERE cached_at < ?", (cutoff_date,))
            self.conn.commit()
            self._update_stats()

    def export_cache_to_parquet(self, output_path):
        with self._lock:
            self.conn.execute("COPY daily_cache TO ? (FORMAT PARQUET)", (output_path,))

    def import_cache_from_parquet(self, input_path):
        with self._lock:
            self.conn.execute("COPY daily_cache FROM ? (FORMAT PARQUET)", (input_path,))
            self._update_stats()
