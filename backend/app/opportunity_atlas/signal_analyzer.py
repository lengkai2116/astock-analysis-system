"""JUD层信号分析模块 — 411号方案Phase 2

从dim1_signal_engine.py迁移全部分析逻辑到JUD层。
dim1不再保留任何分析功能，仅作为数据质量门禁层（Phase 3）。

迁移内容：
  - 7类信号属性分类（classify_attribute）
  - 4维共振评分（calc_resonance_score）
  - 5维度衰减检测（detect_decay）
  - 生命周期阶段计算（calc_lifecycle_stage）
  - 白话文本生成（signal_plain）
  - 条件稽核（build_audit）
  - ConfirmLayer多层验证
"""
...
# ═══════════════════════════════════════════════════════════
# 常量定义（从dim1_signal_engine.py L20-79迁移）
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
# 辅助函数（从dim1_signal_engine.py L86-127迁移）
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
# 衰减检测（从dim1_signal_engine.py L133-201迁移）
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
# 信号属性分类（从dim1_signal_engine.py L208-269迁移）
# ═══════════════════════════════════════════════════════════

def classify_attribute(dims: dict, tags: dict, lifecycle: dict) -> dict:
    """7类信号属性分类（359号§1.3）

    依赖dims（dim2-dim7的输出），必须在dim2-dim7执行完毕后运行。
    """
...
# ═══════════════════════════════════════════════════════════
# 共振评分（从dim1_signal_engine.py L272-290迁移）
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
# 生命周期阶段计算（从dim1_signal_engine.py L297-346迁移）
# ═══════════════════════════════════════════════════════════

def calc_lifecycle_stage(lifecycle: dict) -> dict:
...
# ═══════════════════════════════════════════════════════════
# 条件稽核（从dim1_signal_engine.py L393-440迁移）
# ═══════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════
# ConfirmLayer（从dim1_signal_engine.py L450-664迁移）
# ═══════════════════════════════════════════════════════════
        ...
        Args:
            ...
            context: 上下文数据
                {'turnover_rate': float, 'transfer_speed': float,
                 ...
                 'close_prices': list, 'breakout_price': float}
...
def analyze_signal(dims: dict, tags: dict, lifecycle: dict = None) -> dict:
    """综合信号分析入口（替代原Dim1SignalEngine.evaluate）

    在dim2-dim7执行完毕后调用，依赖它们的输出（dims）。

    Args:
        dims: dim2-dim7的输出格式 {dim_name: {state, light, confidence, evidence}}
        tags: pre_feat_cache标签
        lifecycle: 信号生命周期信息

    Returns:
        {status_description, judgment, audit} — 与原Dim1SignalEngine输出格式兼容
    """
    lifecycle = lifecycle or {}
    ...
    # 4. 生命周期阶段
    lifecycle_info = calc_lifecycle_stage(lifecycle)
