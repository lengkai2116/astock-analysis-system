...
class Dim3VPEngine:
    ...
    def evaluate(self, dims, tags, signals=None, lifecycle=None):
        ...
        # 背离检测
        div_no = str(tags.get('volume_price_fit', '')) == 'diverging'
...
def _classify_granville(df, vol_ratio: float, tags: dict) -> dict:
    ...
    try:
        close = df['close'].astype(float)
        price_chg = (close.iloc[-1] / close.iloc[-2] - 1) * 100 if len(close) >= 2 else 0

        # MA20
        ma20 = close.rolling(20).mean().iloc[-1] if len(close) >= 20 else close.mean()

        # 规则判定
        if price_chg > 1.0 and vol_ratio > 1.5:
            if vol_ratio > 3.0:
        ...
        elif price_chg > 0.5 and vol_ratio > 2.0 and abs(price_chg) < 1.0:
            result = {'rule': 'heavy_pressure', 'name': '放量滞涨', 'description': '多空交锋激烈，上方压力沉重'}

        elif price_chg < -1.0 and vol_ratio > 1.5:
        ...
        elif price_chg < -2.0 and vol_ratio > 1.5 and close.iloc[-1] < ma20:
