...
def plot_chanlun(df, klines_no_contain, fractals, strokes, output_file='chanlun_demo.png'):
    """绘制缠论K线图"""
    ...
    for f in fractals:
        ...
        price = f['price']
        if f['type'] == 'top':
            ax.plot(idx, price, marker='v', color='#ff4757', markersize=8, label='顶' if idx == 0 else "")
            ax.text(idx, price, ' 顶', va='bottom', color='#ff4757', fontweight='bold')
        else:
            ax.plot(idx, price, marker='^', color='#2ed573', markersize=8, label='底' if idx == 0 else "")
            ax.text(idx, price, ' 底', va='top', color='#2ed573', fontweight='bold')
    ...
    for stroke in strokes:
        ...
        ax.plot(x, y, color=color, linewidth=2, 
                label='向上笔' if stroke['direction'] == 'up' else '向下笔' 
                if stroke == strokes[0] else "")
    
    # 美化图表
    ax.set_title('缠论K线图形 - 平安银行(000001.SZ)', fontsize=14, pad=20, fontweight='bold')
    ax.set_xlabel('K线索引', fontsize=11)
...
def main():
    ...
    # 6. 画图
    print("\n🎨 正在生成图表...")
    output_file = '/Users/kalence/Desktop/测试/01-A股股票分析系统/chanlun_demo.png'
    plot_chanlun(df, klines_no_contain, fractals, strokes, output_file)
