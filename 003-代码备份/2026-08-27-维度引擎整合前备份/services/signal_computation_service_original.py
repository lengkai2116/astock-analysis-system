...
class SignalComputationService:
    ...
    # ══════════════════════════════════════════════════════════════════
    # 369号方案 Step10：维度引擎调度层
    # ══════════════════════════════════════════════════════════════════

    def compute_via_engines(self, ts_code: str, tags: dict, dims: dict,
                            lifecycle: dict = None) -> List[Dict]:
        """369号方案：通过维度引擎计算策略信号（替代内联计算）

        调用 dim1-dim7 引擎，将引擎输出转换为与旧接口兼容的信号列表格式。
        """
        signals = []
        try:
            # dim2: 缠论结构信号
            from app.opportunity_atlas.dimensions.dim2_structure_engine import Dim2StructureEngine
            dim2_result = Dim2StructureEngine().evaluate(dims, tags, lifecycle=lifecycle or {})
            sd = dim2_result.get('status_description', {})
            jg = dim2_result.get('judgment', {})
            if sd.get('chanlun_direction') and sd['chanlun_direction'] != '未知':
                signals.append({
                    'strategy_name': '缠论走势分析',
                    'signal': 'bullish' if jg.get('overall_direction', 0) > 0 else ('bearish' if jg.get('overall_direction', 0) < 0 else 'neutral'),
                    'confidence': jg.get('overall_light', 'yellow') == 'green' and 0.7 or 0.4,
                    'evidence': [sd.get('vs_zhongshu', ''), sd.get('vs_ma', '')],
        ...
        try:
            ...
            dim3_result = Dim3VPEngine().evaluate(dims, tags, lifecycle=lifecycle or {})
            sd3 = dim3_result.get('status_description', {})
            jg3 = dim3_result.get('judgment', {})
            if sd3.get('vp_state'):
                signals.append({
                    ...
                    'confidence': jg3.get('score', 5) / 10.0,
        ...
        try:
            ...
            dim4_result = Dim4ChipFundEngine().evaluate(dims, tags, lifecycle=lifecycle or {})
            sd4 = dim4_result.get('status_description', {})
            jg4 = dim4_result.get('judgment', {})
            if sd4.get('phase'):
                signals.append({
                    'strategy_name': '筹码主力分析',
                    'signal': 'bullish' if jg4.get('direction') == 'inflow' else ('bearish' if jg4.get('direction') == 'outflow' else 'neutral'),
                    'confidence': 0.7 if jg4.get('light') == 'green' else 0.4,
        ...
        try:
            ...
            dim5_result = Dim5EmotionEngine().evaluate(dims, tags, lifecycle=lifecycle or {})
            sd5 = dim5_result.get('status_description', {})
            jg5 = dim5_result.get('judgment', {})
            if sd5.get('market'):
                signals.append({
                    ...
                    'confidence': 0.6 if jg5.get('overall_light') == 'green' else 0.4,
